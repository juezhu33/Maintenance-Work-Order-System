...
function MechanicRow({ mechanic }) {
  ...
  async function handleToggle() {
    if (!open && tickets.length === 0) {
      setLoading(true);
      try {
        const data = await apiGetMechanicTickets(mechanic.id);
        setTickets(data);
      } catch (e) {
        console.error(e);
      } finally {
  ...
  return (
    <>
      <TableRow hover>
        ...
        <TableCell>{mechanic.studentNo || "-"}</TableCell>
        <TableCell>{mechanic.fullName}</TableCell>
        <TableCell>{formatTime(mechanic.createdAt)}</TableCell>
      </TableRow>
      <TableRow>
...
export default function AdminMechanicList() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const data = await apiGetUsersByRole("mechanic");
      setList(data);
      setErr("");
    } catch (e) {
        ...
        <Table>
          <TableHead>
            <TableRow>
              ...
              <TableCell sx={{ fontWeight: 700 }}>工号</TableCell>
          ...
          <TableBody>
            {filtered.length === 0 ? (
              ...
              filtered.map((m) => <MechanicRow key={m.id} mechanic={m} />)
            )}
          </TableBody>
        </Table>
      </Paper>
